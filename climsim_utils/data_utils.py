...
class data_utils:
    def __init__(
        ...
        if self.num_latlon == 384:
            ...
            self.lat_bin_dict = {
                lat_bin: self.lat_bin_indices == lat_bin
                for lat_bin in range(len(self.lat_bins) - 1)
            }
            self.lat_bin_area_sums = np.array(
                [
                    np.sum(self.grid_info_area[self.lat_bin_dict[lat_bin]])
                    for lat_bin in self.lat_bin_dict.keys()
                ]
            )
            self.lat_bin_area_divs = np.array(
                [
                    1 / self.lat_bin_area_sums[bin_index]
                    for bin_index in self.lat_bin_indices
                ]
            )
    ...
    def set_to_v1_vars(self):
        ...
        self.ps_index = 120
        self.full_vars = False
        self.input_profile_num = sum(
            1 for input_var in self.input_vars if self.var_lens[input_var] == 60
        )
        self.input_scalar_num = sum(
            1 for input_var in self.input_vars if self.var_lens[input_var] == 1
        )
        self.target_profile_num = sum(
            1 for target_var in self.target_vars if self.var_lens[target_var] == 60
        )
        self.target_scalar_num = sum(
            1 for target_var in self.target_vars if self.var_lens[target_var] == 1
        )
        self.input_feature_len = (
            self.input_profile_num * 60 + self.input_scalar_num
        )  # 124
        self.target_feature_len = (
            self.target_profile_num * 60 + self.target_scalar_num
        )  # 128
    ...
    def get_xrdata(self, file, file_vars=None):
        ...
        if file_vars is not None:
            ...
            if "tm_state_qn_prvphy" in file_vars and "tm_state_qn_prvphy" not in ds:
                tm_state_qn_prvphy = (
                    ds["tm_state_q0002_prvphy"] + ds["tm_state_q0003_prvphy"]
                )
    ...
    def get_target(self, input_file):
        ...
        ds_target = self.get_xrdata(
            input_file.replace(f".{self.input_abbrev}.", f".{self.output_abbrev}.")
        )
        ...
        if self.full_vars:
            ...
            ds_target["ptend_qn"] = (
                ds_target["state_q0002"]
                - ds_input["state_q0002"]
                + ds_target["state_q0003"]
                - ds_input["state_q0003"]
            ) / 1200
            ds_target["ptend_u"] = (
                ds_target["state_u"] - ds_input["state_u"]
            ) / 1200  # U tendency [m/s/s]
        ...
        if data_split == "train":
            ...
            if num_resubmit is not None and num_resubmit_abandon is not None:
                sorted_list = [
                    ...
                    if i % num_resubmit >= num_resubmit_abandon
                ]
            self.train_filelist = sorted_list[
                start_idx : end_idx : self.train_stride_sample
            ]
            if (period is not None) and (samples_to_keep is not None):
                self.train_filelist = [
                    file
                    for i, file in enumerate(self.train_filelist)
                    if i % period < samples_to_keep
                ]
            ...
            self.val_filelist = sorted_list[
                start_idx : end_idx : self.val_stride_sample
            ]
            if (period is not None) and (samples_to_keep is not None):
                self.val_filelist = [
                    file
                    for i, file in enumerate(self.val_filelist)
                    if i % period < samples_to_keep
                ]
            ...
            self.scoring_filelist = sorted_list[
                start_idx : end_idx : self.scoring_stride_sample
            ]
            if (period is not None) and (samples_to_keep is not None):
                self.scoring_filelist = [
                    file
                    for i, file in enumerate(self.scoring_filelist)
                    if i % period < samples_to_keep
                ]
        elif data_split == "test":
            ...
            self.test_filelist = sorted_list[
                start_idx : end_idx : self.test_stride_sample
            ]
            if (period is not None) and (samples_to_keep is not None):
                self.test_filelist = [
                    file
                    for i, file in enumerate(self.test_filelist)
                    if i % period < samples_to_keep
                ]
            ...
            for file in filelist:
                ...
                if self.qinput_log:
                    if "state_q0002" in ds_input:
                        ds_input["state_q0002"][:] = np.log10(
                            1 + ds_input["state_q0002"] * 1e6
                        )
                    if "state_q0003" in ds_input:
                        ds_input["state_q0003"][:] = np.log10(
                            1 + ds_input["state_q0003"] * 1e6
                        )
                ...
                ds_input = ds_input.to_stacked_array(
                    "mlvar", sample_dims=["batch"], name=self.input_abbrev
                )
                # dso = dso.stack({'batch':{'sample','ncol'}})
                ds_target = ds_target.stack({"batch": {"ncol"}})
                ds_target = ds_target.to_stacked_array(
                    "mlvar", sample_dims=["batch"], name=self.output_abbrev
                )
    ...
    def save_as_npy(
        self, data_split, save_path="", save_latlontime_dict=False, precision="float32"
    ):
        """
        ...
        if self.normalize:
            npy_input[np.isinf(npy_input)] = (
                0  # replace inf with 0, some level have constant gas concentration.
            )
        ...
        if self.save_zarr:
            ...
            z = zarr.open(
                zarr_path,
                mode="w",
                shape=npy_input.shape,
                dtype=npy_input.dtype,
                chunks=(100, npy_input.shape[1]),
            )
            z[:] = npy_input
        ...
        if self.save_zarr:
            ...
            z = zarr.open(
                zarr_path,
                mode="w",
                shape=npy_target.shape,
                dtype=npy_target.dtype,
                chunks=(100, npy_target.shape[1]),
            )
            z[:] = npy_target
        ...
        if save_latlontime_dict:
            ...
            latlontime = {
                ...
                for i in range(npy_target.shape[0])
            }
    ...
    def save_as_npy_inputonly(
        self, data_split, save_path="", save_latlontime_dict=False
    ):
        """
        This function saves the training data as a .npy file.
        """
    ...
    def reshape_npy(self, var_arr, var_arr_dim):
        ...
        var_arr = var_arr.reshape(
            (int(var_arr.shape[0] / self.num_latlon), self.num_latlon, var_arr_dim)
        )
        return var_arr
    ...
    def save_norm(self, save_path="", write=False):
        ...
        for var in self.input_vars:
            ...
            if var_lev == 1:
                ...
                input_div.append(
                    self.input_max[var].values - self.input_min[var].values
                )
            else:
                for i in range(var_lev):
                    input_sub.append(self.input_mean[var].values[i])
                    input_div.append(
                        self.input_max[var].values[i] - self.input_min[var].values[i]
                    )
        input_sub = np.array(input_sub)
        input_div = np.array(input_div)
        if write:
            np.savetxt(
                save_path + "/inp_sub.txt",
                input_sub.reshape(1, -1),
                fmt=fmt,
                delimiter=",",
            )
            np.savetxt(
                save_path + "/inp_div.txt",
                input_div.reshape(1, -1),
                fmt=fmt,
                delimiter=",",
            )
        ...
        if write:
            np.savetxt(
                save_path + "/out_scale.txt",
                out_scale.reshape(1, -1),
                fmt=fmt,
                delimiter=",",
            )
        return input_sub, input_div, out_scale

    def save_norm_multirep(
        self,
        per_lev_sub,
        per_lev_div,
        per_col_sub,
        ...
        if write:
            np.savetxt(
                save_path + "/inp_sub_per_lev.txt",
                input_sub_per_lev.reshape(1, -1),
                fmt=fmt,
                delimiter=",",
            )
            np.savetxt(
                save_path + "/inp_div_per_lev.txt",
                input_div_per_lev.reshape(1, -1),
                fmt=fmt,
                delimiter=",",
            )
            np.savetxt(
                save_path + "/inp_sub_per_col.txt",
                input_sub_per_col.reshape(1, -1),
                fmt=fmt,
                delimiter=",",
            )
            np.savetxt(
                save_path + "/inp_div_per_col.txt",
                input_div_per_col.reshape(1, -1),
                fmt=fmt,
                delimiter=",",
            )
            np.savetxt(
                save_path + "/inp_min_norm_per_lev.txt",
                input_min_norm_per_lev.reshape(1, -1),
                fmt=fmt,
                delimiter=",",
            )
        # calculate norms for target
    ...
    def zonal_bin_weight_2d(self, npy_array, custom_weighting=None):
        ...
        if custom_weighting is None:
            npy_array = (
                npy_array
                * self.grid_info_area[None, :]
                * self.lat_bin_area_divs[None, :]
            )
        else:
        ...
        return np.stack(
            [
                np.sum(npy_array[:, self.lat_bin_dict[lat_bin]], axis=1)
                for lat_bin in self.lat_bin_dict.keys()
            ],
            axis=1,
        )
    ...
    def zonal_bin_weight_3d(self, npy_array, custom_weighting=None):
        ...
        if custom_weighting is None:
            npy_array = (
                npy_array
                * self.grid_info_area[None, :, None]
                * self.lat_bin_area_divs[None, :, None]
            )
        else:
        ...
        if data_split == "train":
            ...
            if self.normalize:
                state_ps = (
                    state_ps
                    * (
                        self.input_max["state_ps"].values
                        - self.input_min["state_ps"].values
                    )
                    + self.input_mean["state_ps"].values
                )
            ...
            self.dp_train = (
                self.pressure_grid_train[1:61, :, :]
                - self.pressure_grid_train[0:60, :, :]
            )
            ...
            self.dp_val = (
                self.pressure_grid_val[1:61, :, :] - self.pressure_grid_val[0:60, :, :]
            )
            ...
            self.dp_scoring = (
                self.pressure_grid_scoring[1:61, :, :]
                - self.pressure_grid_scoring[0:60, :, :]
            )
            ...
            self.dp_test = (
                self.pressure_grid_test[1:61, :, :]
                - self.pressure_grid_test[0:60, :, :]
            )
    ...
    def output_weighting(self, output, data_split, just_weights=False):
        ...
        if not self.full_vars and not self.microphysics_constraint:
            ...
            ptend_qn = output[:, 120:180].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon, 60)
            )
            ptend_u = output[:, 180:240].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon, 60)
            )
            ptend_v = output[:, 240:300].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon, 60)
            )
            netsw = output[:, 300].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon)
            )
            flwds = output[:, 301].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon)
            )
            precsc = output[:, 302].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon)
            )
            precc = output[:, 303].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon)
            )
            sols = output[:, 304].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon)
            )
            soll = output[:, 305].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon)
            )
            solsd = output[:, 306].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon)
            )
            solld = output[:, 307].reshape(
                (int(num_samples / self.num_latlon), self.num_latlon)
            )
            ...
            if just_weights:
                ...
                ptend_qn_weight = weightings[:, 120:180].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon, 60)
                )
                ptend_u_weight = weightings[:, 180:240].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon, 60)
                )
                ptend_v_weight = weightings[:, 240:300].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon, 60)
                )
                netsw_weight = weightings[:, 300].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon)
                )
                flwds_weight = weightings[:, 301].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon)
                )
                precsc_weight = weightings[:, 302].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon)
                )
                precc_weight = weightings[:, 303].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon)
                )
                sols_weight = weightings[:, 304].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon)
                )
                soll_weight = weightings[:, 305].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon)
                )
                solsd_weight = weightings[:, 306].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon)
                )
                solld_weight = weightings[:, 307].reshape(
                    (int(num_samples / self.num_latlon), self.num_latlon)
                )
        else:
            ptend_t = output[:, :60].reshape(
        ...
        if self.normalize:
            ...
            if self.microphysics_constraint:
                ptend_qn = (
                    ptend_qn
                    / self.output_scale["ptend_qn"].values[np.newaxis, np.newaxis, :]
                )
                ptend_u = (
                    ptend_u
                    / self.output_scale["ptend_u"].values[np.newaxis, np.newaxis, :]
                ...
                if just_weights:
                    ptend_qn_weight = (
                        ptend_qn_weight
                        / self.output_scale["ptend_qn"].values[
                            np.newaxis, np.newaxis, :
                        ]
                    )
                    ptend_u_weight = (
        ...
        if self.microphysics_constraint:
            ...
            if just_weights:
                ptend_qn_weight = (
                    ptend_qn_weight * self.area_wgt[np.newaxis, :, np.newaxis]
                )
                ptend_u_weight = (
                    ptend_u_weight * self.area_wgt[np.newaxis, :, np.newaxis]
                )
    ...
    def reweight_target(self, data_split):
        ...
        if data_split == "train":
            ...
            self.target_weighted_train = self.output_weighting(
                self.target_train, data_split
            )
        elif data_split == "val":
            assert self.target_val is not None
            self.target_weighted_val = self.output_weighting(
                self.target_val, data_split
            )
        elif data_split == "scoring":
            assert self.target_scoring is not None
            self.target_weighted_scoring = self.output_weighting(
                self.target_scoring, data_split
            )
        elif data_split == "test":
            assert self.target_test is not None
            self.target_weighted_test = self.output_weighting(
                self.target_test, data_split
            )
    ...
    def reweight_preds(self, data_split):
        ...
        if data_split == "train":
            ...
            for model_name in self.model_names:
                self.preds_weighted_train[model_name] = self.output_weighting(
                    self.preds_train[model_name], data_split
                )
        elif data_split == "val":
            assert self.preds_val is not None
            for model_name in self.model_names:
                self.preds_weighted_val[model_name] = self.output_weighting(
                    self.preds_val[model_name], data_split
                )
        elif data_split == "scoring":
            assert self.preds_scoring is not None
            for model_name in self.model_names:
                self.preds_weighted_scoring[model_name] = self.output_weighting(
                    self.preds_scoring[model_name], data_split
                )
        elif data_split == "test":
            assert self.preds_test is not None
            for model_name in self.model_names:
                self.preds_weighted_test[model_name] = self.output_weighting(
                    self.preds_test[model_name], data_split
                )
    ...
    def reweight_samplepreds(self, data_split):
        ...
        if data_split == "train":
            ...
            for model_name in self.model_names:
                self.samplepreds_weighted_train[model_name] = (
                    self.output_weighting_CRPS(
                        self.samplepreds_train[model_name], data_split
                    )
                )
        elif data_split == "val":
            assert self.samplepreds_val is not None
            for model_name in self.model_names:
                self.samplepreds_weighted_val[model_name] = self.output_weighting_CRPS(
                    self.samplepreds_val[model_name], data_split
                )
        elif data_split == "scoring":
            assert self.samplepreds_scoring is not None
            for model_name in self.model_names:
                self.samplepreds_weighted_scoring[model_name] = (
                    self.output_weighting_CRPS(
                        self.samplepreds_scoring[model_name], data_split
                    )
                )
        elif data_split == "test":
            assert self.samplepreds_test is not None
            for model_name in self.model_names:
                self.samplepreds_weighted_test[model_name] = self.output_weighting_CRPS(
                    self.samplepreds_test[model_name], data_split
                )
    ...
    def calc_CRPS(self, samplepreds, target, avg_grid=True):
        ...
        spread = (diff * count[np.newaxis, np.newaxis, np.newaxis, :]).mean(
            axis=(0, -1)
        )  # mean over time and crps samples
    ...
    def create_metrics_df(self, data_split):
        ...
        if data_split == "train":
            ...
            for model_name in self.model_names:
                df_var = pd.DataFrame(
                    columns=self.metrics_names, index=self.target_vars
                )
                df_var.index.name = "variable"
                df_idx = pd.DataFrame(
                    columns=self.metrics_names, index=range(self.target_feature_len)
                )
                ...
                for metric_name in self.metrics_names:
                    ...
                    for target_var in self.target_vars:
                        metric = self.metrics_dict[metric_name](
                            self.preds_weighted_train[model_name][target_var],
                            self.target_weighted_train[target_var],
                        )
                        df_var.loc[target_var, metric_name] = np.mean(metric)
                        df_idx.loc[
                            current_idx : current_idx + self.var_lens[target_var] - 1,
                            metric_name,
                        ] = np.atleast_1d(metric)
            ...
            for model_name in self.model_names:
                ...
                for metric_name in self.metrics_names:
                    ...
                    for target_var in self.target_vars:
                        metric = self.metrics_dict[metric_name](
                            self.preds_weighted_val[model_name][target_var],
                            self.target_weighted_val[target_var],
                        )
                        df_var.loc[target_var, metric_name] = np.mean(metric)
                        df_idx.loc[
                            current_idx : current_idx + self.var_lens[target_var] - 1,
            ...
            for model_name in self.model_names:
                ...
                for metric_name in self.metrics_names:
                    ...
                    for target_var in self.target_vars:
                        metric = self.metrics_dict[metric_name](
                            self.preds_weighted_scoring[model_name][target_var],
                            self.target_weighted_scoring[target_var],
                        )
                        df_var.loc[target_var, metric_name] = np.mean(metric)
                        df_idx.loc[
                            current_idx : current_idx + self.var_lens[target_var] - 1,
            ...
            for model_name in self.model_names:
                ...
                for metric_name in self.metrics_names:
                    ...
                    for target_var in self.target_vars:
                        metric = self.metrics_dict[metric_name](
                            self.preds_weighted_test[model_name][target_var],
                            self.target_weighted_test[target_var],
                        )
                        df_var.loc[target_var, metric_name] = np.mean(metric)
                        df_idx.loc[
                            current_idx : current_idx + self.var_lens[target_var] - 1,
