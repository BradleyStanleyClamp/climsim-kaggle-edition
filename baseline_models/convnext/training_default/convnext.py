...
class ConvNeXt(modulus.Module):
    ...
    def forward(self, x):
        x_profile = x[:, : self.input_profile_num * self.vertical_level_num].reshape(
            -1, self.input_profile_num, self.vertical_level_num
        )
        x_scalar = (
            x[:, self.input_profile_num * self.vertical_level_num :]
            .unsqueeze(2)
            .repeat(1, 1, self.vertical_level_num)
        )
        ...
        profile_part = profile_part.reshape(
            -1, self.target_profile_num * self.vertical_level_num
        )  # b,300
        scalar_part = outputs[:, self.target_profile_num :, :]
        scalar_part = self.fc_scalar(
            scalar_part.reshape(-1, self.target_scalar_num * self.vertical_level_num)
        )  # b, 8
