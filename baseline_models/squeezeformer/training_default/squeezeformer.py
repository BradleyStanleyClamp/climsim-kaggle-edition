...
class Squeezeformer(modulus.Module):
    def __init__(
        ...
        self.dense = nn.Linear(
            self.input_profile_num + self.input_scalar_num, self.embed_dim, bias=False
        )
        self.layer_norm = nn.LayerNorm(self.embed_dim, eps=1e-6)
        self.conv_blocks = nn.ModuleList(
            [
                Conv1DBlockSqueezeformer(
                    channel_size=self.embed_dim,
        ...
        self.ffn = GLUMlp(expand_dim=self.expand_dim, head_dim=self.head_dim)
        self.final_dense_pred = nn.Linear(
    ...
    def forward(self, x):
        # reshape input
        profile_part = (
            x[:, : self.input_profile_num * self.vertical_level_num]
            .reshape(-1, self.input_profile_num, self.vertical_level_num)
            .transpose(1, 2)
        )
        scalar_part = (
            x[:, self.input_profile_num * self.vertical_level_num :]
            .unsqueeze(1)
            .repeat(1, self.vertical_level_num, 1)
        )
        x = torch.cat([profile_part, scalar_part], dim=-1)  # b, 60, 26

        x = self.dense(x)
        x = self.layer_norm(x)
        for conv_block, transformer_encoder in zip(
            self.conv_blocks, self.transformer_encoders
        ):
            x = conv_block(x)
        ...
        profile_part = profile_part.permute(0, 2, 1).reshape(
            -1, self.target_profile_num * self.vertical_level_num
        )  # b,300
