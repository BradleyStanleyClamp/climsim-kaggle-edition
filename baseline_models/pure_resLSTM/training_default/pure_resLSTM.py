...
class PureResLSTM(modulus.Module):
    def __init__(
        ...
        for i in range(num_lstm):
            lstm_key = f"lstm{i + 1}"
            lstm_layers[lstm_key] = nn.LSTM(
                input_size=self.inputs_dim if i == 0 else self.hidden_state * 2,
            ...
            if i == 0:
                residual_layers.append(
                    nn.Sequential(
                        nn.LayerNorm([self.vertical_level_num, self.hidden_state * 2]),
                        nn.GELU(),
                    )
                )
            else:
    ...
    def forward(self, x):
        ...
        for i, (lstm, residual) in enumerate(
            zip(self.lstm_stack.values(), self.residual_stack)
        ):
            outputs, _ = lstm(outputs)
            outputs = residual(outputs)
            if i > 0:
                outputs = self.res_act(
                    0.7 * outputs + 0.3 * last_outputs
                )  # residual connection
                last_outputs = outputs
            else:  # i % 2 ==0
