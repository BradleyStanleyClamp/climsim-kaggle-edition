...
class EncDecLSTM(modulus.Module):
    def __init__(
        ...
        self.output_dim = (
            self.target_profile_num * self.vertical_level_num + self.target_scalar_num
        )
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
        self.encoder = MLP(
            [
                self.num_input_vars * self.vertical_level_num,
                self.num_input_vars * 15,
                self.num_input_vars * 8,
                self.num_input_vars * 4,
            ]
        )
        self.decoder = MLP(
            [
        ...
        self.lstm = nn.LSTM(
            input_size=self.num_input_vars * 2,
    ...
    def forward(self, x):
        ...
        # flatten to b, 60 * 26
        x_enc = self.encoder(
            x.reshape(-1, self.num_input_vars * self.vertical_level_num)
        )
        x_dec = self.decoder(x_enc).reshape(
            -1, self.vertical_level_num, self.num_input_vars
        )
